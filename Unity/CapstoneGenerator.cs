...
static class Variations
{
    ...
    public static void UseRandGen(System.Random randGen)
    {
        rand = randGen;
    }
...
class HookerUpper
{
    ...
    public void SetMeshValues(Mesh mesh, Vector3[] positions, Vector3[] normals, Vector2[] uvs)
    {
        mesh.Clear();
    ...
    {
        const int fpv = 12;
        const int normalOffset = 9;
        const int uvOffset = 7;

        Vector3[] positions, normals;
        Vector2[] uvs;

        VariationMath.OffsetTransformVertsIntoArrays(Vector3.zero, Matrix4x4.identity, out positions, out normals, out uvs, numVertices, fpv, normalOffset, uvOffset, baseVerts);
        ...
        SetMeshValues(mesh, positions, normals, uvs);
    }
}
...
{
    public static void OffsetTransformVertsIntoArrays(Vector3 offset, Matrix4x4 transform, out Vector3[] positions, out Vector3[] normals, out Vector2[] uvs, int vertexCount, int floatsPerVertex, int normalOffset, int uvOffset, float[] vertices)
	{
        ...
        for (int i = 0; i < vertexCount; ++i)
		{
            ...
            uvs[i] = new Vector2(vertices[uvIndex + 0], vertices[uvIndex + 1]);
        }
	}
}
...
public class CapstoneGenerator : MonoBehaviour {
    ...
    private int m_normalOffset = 9;
    private int m_uvOffset = 7;
    private HookerUpper m_hookerUpper;

    public bool Regening { get; private set; }
    ...
    private void MakeMesh(ComponentRef reference)
    {
        ...
        reference.m_renderer.material.color = Color.Lerp(m_minColor, m_maxColor, Variations.FRand());
    ...
    private void MakeBaseMesh(ComponentRef reference)
    {
        ...
        Vector3[] positions, normals;
        Vector2[] uvs;
        VariationMath.OffsetTransformVertsIntoArrays(Vector3.zero, Matrix4x4.identity, out positions, out normals, out uvs, m_numVertices, m_floatsPerVertex, m_normalOffset, m_uvOffset, m_baseVerts);
        m_hookerUpper.SetMeshValues(reference.m_mesh, positions, normals, uvs);
        m_objectVariations.ClearVariations();
    }

    public void RegenWithRandGen(System.Random randGen)
    {
        if (!Regening)
        {
            Variations.UseRandGen(randGen);
            StartCoroutine(ReMakeObjects(m_genStep));
        }
    }

    public void Regen()
    {
        if (!Regening) { StartCoroutine(ReMakeObjects(m_genStep)); }
    }

    public void BoringIfy()
    {
        if (!Regening) { StartCoroutine(BoringifyObjects(m_genStep)); }
    }
